...
# Configure logging (add after app initialization)
...
@app.route('/login')
def login():
    # before redirecting, print state in session
...
def protected():
    if 'profile' not in session:
        app.logger.warning('Unauthorized access attempt', extra={
            'timestamp': datetime.utcnow().isoformat()
        })
        return redirect('/login')
    app.logger.info('Protected access', extra={
